...
#End If
    ...
    ''' 设备唯一标识符。
    ''' </summary>
    Public UniqueAddress As String = ModBase.GetUniqueAddress()
    ''' <summary>
    ''' 程序是否已结束。
    ''' </summary>
...
#End Region
    ...
    Private Function EnableIterator(asset As String) As String
        Dim result As String
        If String.IsNullOrEmpty(asset) Then
            result = "@;$ Abv2"
        Else
    ...
    Public Function ValidateIterator(instance As String, Optional second As String = "") As String
        second = ModBase.EnableIterator(second)
        Dim bytes As Byte() = Encoding.UTF8.GetBytes(second)
        ...
        Using memoryStream As MemoryStream = New MemoryStream()
            Dim bytes3 As Byte() = Encoding.UTF8.GetBytes(instance)
    ...
    Public Function CancelIterator(v As String, Optional selection As String = "") As String
        selection = ModBase.EnableIterator(selection)
        Dim bytes As Byte() = Encoding.UTF8.GetBytes(selection)
        Dim bytes2 As Byte() = Encoding.UTF8.GetBytes("95168702")
        Dim descryptoServiceProvider As DESCryptoServiceProvider = New DESCryptoServiceProvider()
        Dim [string] As String
        Using memoryStream As MemoryStream = New MemoryStream()
            Dim array As Byte() = Convert.FromBase64String(v)
    ...
    Public Function ResolveIterator(task As String, visitor As String) As Boolean
        Dim result As Boolean
        Try
            Dim rsacryptoServiceProvider As RSACryptoServiceProvider = New RSACryptoServiceProvider(&H200)
            rsacryptoServiceProvider.FromXmlString("<RS!AKeyValu!e><Mo!dul!us>0L/cZoJUyBRAIE8OKiG8$qYOytXD$azFCBsmOuQra".Replace("!", "").Replace("$", "+") + "/R1Frckd3/Sn+Zsx9aD6U2f" + Conversions.ToString(Math.Round(84D)) + "SdWMDlrRY9DfhQ==</Modulus><Exponent>AQAB<\Exponent><\RSAKeyValue>".Replace("\", "/"))
            result = rsacryptoServiceProvider.VerifyData(Encoding.[Default].GetBytes(task), GetType(SHA256), Convert.FromBase64String(visitor))
        Catch ex As Exception
            result = False
        End Try
        Return result
    ...
    Public Function GetUniqueAddress() As String
        Return "0"
    End Function
